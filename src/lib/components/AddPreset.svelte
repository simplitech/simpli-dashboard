<script lang="ts">
  import type { Preset, SelectedFilters } from '$lib/utils/format'
  import { showToast } from '$lib/utils/toast'
  import Popover from './Popover.svelte'

  export let selectedFilters: SelectedFilters
...
<Popover bind:isOpen>
  ...
  <div class="w-[228px] bg-purple-gray-300 flex flex-col gap-y-2 rounded-md py-3 px-2" slot="content">
    <input
      type="text"
      class="w-full h-5 text-white italic p-4 rounded-full bg-purple-gray-400 border border-light-gray focus:outline-none focus:border-lilac focus:border-2"
